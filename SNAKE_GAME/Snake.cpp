#include "Snake.h"

Snake::Snake()
{
    //ctor
}

Snake::~Snake()
{
    //dtor
}

void Snake::Init(){
    BaseObject initHead, initTail;
    initHead.RandomGenerate();
    initTail = initHead;
    initTail.Move(CELL_SIZE*dx[initHead.getDIR()], CELL_SIZE*dy[initHead.getDIR()]);

    segment.push_back(initHead);
    segment.push_back(initTail);

    for(int i = 0; i < 12; i++){
        Head[i] = NULL;
    }
    for(int i = 0; i < 8; i++){
        Body[i] = NULL;
    }
    for(int i = 0; i < 4; i++){
        Tail[i] = NULL;
    }
}

bool Snake::LoadImages(SDL_Renderer* renderer){
    //Head
    for(int i = 0; i < 4; i++){
        for(int j = 1; j <= 3; j++){
            string src = "Image/Head";
            src += cDIR[i];
            src += char(j+'0');
            src += ".png";
            Head[i*3+j-1] = loadTexture(renderer,  src);
//            cerr<<Src<<'\n';
            if(Head[i*3+j-1] == NULL){
                cerr<<src<<'\n';
                return false;
            }
        }
    }

    //Body
    Body[0] = loadTexture(renderer, "Image/BodyU.png");
    if(Body[0] == NULL){
        cerr<<"Image/BodyU.png";
        return false;
    }

    Body[1] = loadTexture(renderer, "Image/BodyL.png");
    if(Body[1] == NULL){
        cerr<<"Image/BodyL.png";
        return false;
    }

    for(int i = 0; i < 2; i++){
        for(int j = 2; j < 4; j++){
            string src = "Image/Body";
            src += cDIR[i];
            src += cDIR[j];
            src += ".png";
            Body[i*2+j] = loadTexture(renderer, src);
            if(Body[i*2+j] == NULL){
                return false;
            }
        }
    }

    //Tail
    for(int i=0; i<4; i++){
        string src = "Image/Tail";
        src += cDIR[i];
        src += ".png";
        Tail[i] = loadTexture(renderer, src);
        if(Tail[i] == NULL){
            cerr<<src<<'\n';
            return false;
        }
    }

    return true;
}

bool Snake::CollideWithBody()
{
    for (int i = 1; i+1 < segment.size(); ++i) {
        if (segment.front() == segment[i]) {
            return true;
        }
    }
    return false;
}

bool Snake::CollideWithObject(BaseObject Object)
{
     for (int i = 1; i+1 < segment.size(); ++i) {
        if (segment[i] == Object) {
            return true;
        }
    }
    return false;
}

void Snake::NextStep(){
    BaseObject newHead = segment.front();
    newHead.Move(CELL_SIZE * dx[newHead.getDIR()],
                         CELL_SIZE * dy[newHead.getDIR()]);

    //go through the edge
    if(newHead.getX() < 0) newHead.Move(SCREEN_WIDTH, 0);
    else if(newHead.getX() == SCREEN_WIDTH) newHead.setX(0);

    if(newHead.getY() < 0) newHead.Move(0, SCREEN_HEIGHT);
    else if(newHead.getY() == SCREEN_HEIGHT) newHead.setY(0);

    segment.push_front(newHead);
}

void Snake::Popback(){
    segment.pop_back();
}

void Snake::Draw(SDL_Renderer* renderer, BaseObject& food){

    for (int i = 1; i + 1 < segment.size(); i++) {

        int nxtDir = segment[i+1].getDIR();
        int curDir = segment[i].getDIR();

        int id;
        if(curDir == nxtDir){
            id = (curDir < 2 ? 0 : 1);
        }
        else{
            // noi giua huong hien tai va huong nguoc lai cua huong sau do
            int reverseNxtDir = nxtDir ^ 1;
            int UD_Dir = min(reverseNxtDir, curDir);
            int LR_Dir = max(reverseNxtDir, curDir);
            id = UD_Dir * 2 + LR_Dir;
        }

        applyImage(renderer, Body[id], segment[i].getX(), segment[i].getY(), CELL_SIZE, CELL_SIZE);
    }
    //draw tail
    applyImage(renderer, Tail[segment.back().getDIR()], segment.back().getX(), segment.back().getY(), CELL_SIZE, CELL_SIZE);

    //draw head
    int manhattanDis = (abs(segment.front().getX() - food.getX()) + abs(segment.front() .getY() - food.getY())) / CELL_SIZE;
    int id = segment.front().getDIR()*3;
    if(manhattanDis == 0 || manhattanDis == 2) id++;
    if(manhattanDis == 1) id += 2;
    applyImage(renderer, Head[id], segment.front().getX()-5, segment.front().getY()-5, CELL_SIZE+10, CELL_SIZE+10);
}

void Snake::HandleEvent(SDL_Event event){
    int DIR = segment.front().getDIR();
    int newDIR = DIR;
    if(event.type == SDL_KEYDOWN){
        switch(event.key.keysym.sym){
            case SDLK_UP:
            if(DIR != DIR_DOWN) newDIR = DIR_UP;
            break;

            case SDLK_DOWN:
            if(DIR != DIR_UP) newDIR = DIR_DOWN;
            break;

            case SDLK_LEFT:
            if(DIR != DIR_RIGHT) newDIR = DIR_LEFT;
            break;

            case SDLK_RIGHT:
            if(DIR != DIR_LEFT) newDIR = DIR_RIGHT;
            break;

        }
    }

    segment.front().setDIR(newDIR);
}
